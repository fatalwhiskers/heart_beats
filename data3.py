...
def extract_rgb_poly(video_path: str, crop_mode: str = "poly", display: bool = False, testing: bool = False
    ...
    try:
        R, G, B, t = VE.extract_video_to_rgb(
            video_path,
            crop_mode=crop_mode,
            Display=display,
            Testing=testing
        ...
        try:
            R, G, B, t = VE.extract_video_to_rgb(video_path, 487, 263, 722, 498, crop_mode, display, testing)
            return np.asarray(R), np.asarray(G), np.asarray(B), np.asarray(t)
        except Exception as e:
            raise RuntimeError(f"Could not call VE.extract_video_to_rgb: {e}")
